...
function runRRT(numRuns::Int; postProcess::Bool=false)
    times = zeros(numRuns)
    ΔvVec = zeros(numRuns)
...
function runRealTimeRRT(runRate::AbstractFloat)
    ...
    while success
        #Check to see if new pathfinding is required, run RRT if soln
        if abs(t % 10) < 0.999
...
dvs = zeros(10)
for i in 1:10
    temp, path, dvs[i] = runRealTimeRRT(3600.0)
...
#= Things to consider...
Only re-run RRT if current soln to next waypoint is currently blocked.
In post-processing, heavy weight to reduce max timestep or increase min timestep
    argmax/argmin
Try to find why there are big timejumps sometimes
=#